...
uint16_t crc(uint16_t checksum, pspcommsg msg)
{
    ...
    uint8_t *msg_content = (uint8_t*) malloc(6 + msg.payload_len);
    msg_content[0] = '!';
    msg_content[1] = '$';
    msg_content[2] = (uint8_t) msg.payload_len;
    msg_content[3] = (uint8_t) msg.device_id;
    msg_content[4] = (uint8_t) (msg.device_id >> 8);
    msg_content[5] = (uint8_t) msg.msg_id;
    for(int i = 0; i < msg.payload_len; i++)
    {
        msg_content[6 + i] = msg.payload[i];
    }
    for (int i = 0; i < 6 + msg.payload_len; i++) {
